...
===========================
Calculate yearly returns, drawdowns, and Sharpe ratios
"""
...
for year in sorted(strategy_series.index.year.unique()):
    ...
    sharpe = (annual_ret / annual_vol * np.sqrt(252)) if annual_vol != 0 else np.nan

    max_dd = (year_wealth / year_wealth.cummax() - 1).min()

    downside = year_ret[year_ret < 0]
    downside_std = downside.std() if len(downside) > 0 else 0
    sortino = (annual_ret / downside_std * np.sqrt(252)) if downside_std != 0 else np.nan
...
print("\n" + "="*140)
print("YEARLY PERFORMANCE METRICS (1997-2026)")
print("="*140)

display_df = yearly_df.copy()
display_df['Return'] = display_df['Return'].apply(lambda x: f"{x:.2%}")
display_df['Sharpe'] = display_df['Sharpe'].apply(lambda x: f"{x:.4f}")
display_df['Max_Drawdown'] = display_df['Max_Drawdown'].apply(lambda x: f"{x:.2%}")
display_df['Sortino'] = display_df['Sortino'].apply(lambda x: f"{x:.4f}")
...
Negative Years: {(yearly_numeric['Return'] < 0).sum()}/{len(yearly_numeric)}
...
RETURNS:
  ...
  Median: {yearly_numeric['Return'].median():.2%}
  Std Dev: {yearly_numeric['Return'].std():.2%}
...
DRAWDOWNS:
  ...
  Best (least negative): {yearly_numeric['Max_Drawdown'].max():.2%}
  Average: {yearly_numeric['Max_Drawdown'].mean():.2%}

SORTINO RATIOS:
  Best: {yearly_numeric['Sortino'].max():.4f}
  Worst: {yearly_numeric['Sortino'].min():.4f}
  Average: {yearly_numeric['Sortino'].mean():.4f}
""")
...
axes[1].set_title('Yearly Sharpe Ratios', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('results/yearly_performance_metrics.png', dpi=150, bbox_inches='tight')
print(f"\nChart saved: results/yearly_performance_metrics.png")

yearly_df.to_csv('results/yearly_performance_detailed.csv', index=False)
print(f"Data saved: results/yearly_performance_detailed.csv")
