...
def run_momentum_bubble_hedge_and_low_bubble_leverage(
    ...
    two_year_col = find_col(close, ["2Y", "US2Y", "^FVX", "DGS2"])
    ten_year_col = find_col(close, ["10Y", "US10Y", "^TNX", "DGS10"])

    print("2Y column:", two_year_col)
    print("10Y column:", ten_year_col)
    ...
    best_treasury_bubble_df = None
    best_signal_treasury_snapshot = None
    best_params = None
    ...
    for (
        ...
        temp_ret = temp_ret_df["Total"].rename(
            "Momentum_HighBubbleHedge_LowBubbleLeverage"
        )
        ...
        if pd.notna(temp_sharpe) and temp_sharpe > best_sharpe:
            ...
            best_hedge_exposure = pd.Series(
                hedge_exposure,
                index=ret_df.index,
                name="UVXY_Hedge_Exposure"
            )

            best_leverage_exposure = pd.Series(
                leverage_exposure,
                index=ret_df.index,
                name="Momentum_Extra_Leverage"
            )

            best_hedge_signal = hedge_trade_signal.rename(
                "Hedge_Trade_Signal_Next_Day"
            )

            best_leverage_signal = leverage_trade_signal.rename(
                "Momentum_Leverage_Signal_Next_Day"
            )

            best_bubble_score = bubble_score.rename("BubbleScore_Proxy")
            best_params = row

            treasury_bubble_parts = {}

            if two_year_col is not None:
                two_year_price = close.loc[ret_df.index, two_year_col].replace(0, np.nan).ffill()
                treasury_bubble_parts["Two_Year_BubbleScore"] = calculate_bubble_score_proxy(
                    two_year_price,
                    ma_window=ma_window,
                    z_window=z_window,
                )

            if ten_year_col is not None:
                ten_year_price = close.loc[ret_df.index, ten_year_col].replace(0, np.nan).ffill()
                treasury_bubble_parts["Ten_Year_BubbleScore"] = calculate_bubble_score_proxy(
                    ten_year_price,
                    ma_window=ma_window,
                    z_window=z_window,
                )

            if len(treasury_bubble_parts) > 0:
                best_treasury_bubble_df = pd.concat(treasury_bubble_parts, axis=1)

                best_signal_treasury_snapshot = best_treasury_bubble_df.copy()
                best_signal_treasury_snapshot["Momentum_Leverage_Entry"] = (
                    best_leverage_signal == True
                )
                best_signal_treasury_snapshot["Long_UVXY_Hedge_Entry"] = (
                    best_hedge_signal == True
                )

                best_signal_treasury_snapshot = best_signal_treasury_snapshot[
                    best_signal_treasury_snapshot["Momentum_Leverage_Entry"]
                    | best_signal_treasury_snapshot["Long_UVXY_Hedge_Entry"]
                ]
    ...
    ax2.plot(
        best_bubble_score.index,
        best_bubble_score,
        label=f"BubbleScore Proxy: {best_params['bubble_indicator']}"
    )

    ax2.axhline(
        best_params["hedge_bubble_entry"],
        linestyle="--",
        label="High Bubble Hedge Entry"
    )

    ax2.axhline(
        best_params["low_bubble_entry"],
        linestyle="--",
        label="Low Bubble Leverage Entry"
    )
    ...
    ax3.plot(
        best_hedge_exposure.index,
        best_hedge_exposure,
        label="Long UVXY / Synthetic VIX Hedge Exposure"
    )
    ...
    ax4.plot(
        best_leverage_exposure.index,
        best_leverage_exposure,
        label="Extra Momentum Leverage"
    )
    ...
    for col in [
        ...
        ax5.plot(
            best_decomposition_wealth.index,
            best_decomposition_wealth[col],
            label=col,
            linewidth=2
        )
    ...
    if best_treasury_bubble_df is not None:
        fig2, ax = plt.subplots(figsize=(24, 10))

        for col in best_treasury_bubble_df.columns:
            ax.plot(
                best_treasury_bubble_df.index,
                best_treasury_bubble_df[col],
                label=col,
                linewidth=2.5,
                zorder=2
            )

        leverage_entry_dates = best_leverage_signal[
            best_leverage_signal == True
        ].index

        hedge_entry_dates = best_hedge_signal[
            best_hedge_signal == True
        ].index

        leverage_entry_dates = leverage_entry_dates.intersection(
            best_treasury_bubble_df.index
        )

        hedge_entry_dates = hedge_entry_dates.intersection(
            best_treasury_bubble_df.index
        )

        treasury_marker_level = best_treasury_bubble_df.mean(axis=1)

        if len(leverage_entry_dates) > 0:
            ax.scatter(
                leverage_entry_dates,
                treasury_marker_level.loc[leverage_entry_dates],
                label="Momentum Leverage Entry",
                marker="^",
                s=260,
                edgecolors="black",
                linewidths=1.8,
        ...
        if len(hedge_entry_dates) > 0:
            ax.scatter(
                hedge_entry_dates,
                treasury_marker_level.loc[hedge_entry_dates],
                label="Long UVXY Hedge Entry",
                marker="v",
                s=260,
                edgecolors="black",
                linewidths=1.8,
        ...
        ax.axhline(0, linestyle="--", linewidth=1.5, zorder=1)
        ax.set_ylim(-1.05, 1.05)

        ax.set_title(
            "2Y / 10Y Treasury BubbleScore at Entry Points",
            fontsize=20
        )
        ax.set_ylabel("Treasury BubbleScore", fontsize=15)
...
results = run_momentum_bubble_hedge_and_low_bubble_leverage(df4_mom)
...
grid_result_df.to_csv("grid_result_df.csv")

best_return_decomposition.to_csv("best_return_decomposition.csv")
best_decomposition_wealth.to_csv("best_decomposition_wealth.csv")
best_hedge_exposure.to_csv("best_hedge_exposure.csv")
best_leverage_exposure.to_csv("best_leverage_exposure.csv")
best_bubble_score.to_csv("best_bubble_score.csv")
best_hedge_signal.to_csv("best_hedge_signal.csv")
best_leverage_signal.to_csv("best_leverage_signal.csv")
hedge_source_series.to_csv("hedge_source_series.csv")

if best_treasury_bubble_df is not None:
    best_treasury_bubble_df.to_csv("best_treasury_bubble_df.csv")

if best_signal_treasury_snapshot is not None:
    best_signal_treasury_snapshot.to_csv("best_signal_treasury_snapshot.csv")