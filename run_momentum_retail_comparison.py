"""
Good Retail Execution Comparison: Hourly vs Daily Momentum
===========================================================
TC = 0.2% round-trip (0.1% one-way) - typical for good retail execution
...
Strategy A -- Hourly Momentum (Long-Only)
  ...
  TC        : 0.1% per trade (0.2% round-trip)
...
_n = [0]
def _save(*a, **k):
    _n[0] += 1; p = f"results/retail_compare_chart_{_n[0]}.png"
...
TC_RETAIL = 0.001  # 0.1% one-way = 0.2% round-trip
BORROW_HOURLY_RETAIL = 0.08 / TRADING_HOURS
...
# ══════════════════════════════════════════════════════════════════════════
# STEP 1 -- Hourly Momentum Grid (Good Retail TC = 0.1%)
# ══════════════════════════════════════════════════════════════════════════

_header("Strategy A -- Hourly Momentum (Grid, TC=0.1% one-way)")
...
print("Precomputing momentum...", flush=True)
mom_cache = {}
for lb in LB_GRID:
...
print(f"{'LB':>6} {'Hold':>6} {'N':>4} | {'Sharpe':>7} {'Sortino':>8} {'Return':>9} {'MaxDD':>8}")
print("-" * 55)
...
if best_h_params:
    ...
    print(f"Sharpe={best_h_sh:.3f}")


# ══════════════════════════════════════════════════════════════════════════
# STEP 2 -- Daily Momentum (140d/40d, TC=0.1%)
# ══════════════════════════════════════════════════════════════════════════

_header("Strategy B -- Daily Momentum (140d/40d, TC=0.1%)")
...
for top_n in TOP_N_DAILY:
    ...
    while i < len(daily_close) - 1:
        ...
        tc_cost = TC_RETAIL * n_changes / top_n
...
_header("HEAD-TO-HEAD COMPARISON (Good Retail, TC=0.1% one-way)")
...
if best_h_ret is not None and best_d_ret is not None:
    ...
    for metric in ["Sharpe", "Sortino", "Total Return", "Max DD"]:
        ...
        winner = " [HOURLY]" if hv > dv else " [DAILY]" if dv > hv else " [TIE]"
        print(f"{metric:<22} {hv:>18{fmt}} {dv:>18{fmt}}{winner}")
    ...
    for label, s, col in [
        (f"Hourly Momentum (lb={best_h_params['lookback']}h "
         f"hold={best_h_params['hold']}h n={best_h_params['top_n']}, TC=0.1%)",
         h_ret, colors["Hourly"]),
        (f"Daily Momentum (lb={LOOKBACK_D}d hold={HOLD_D}d n={best_d_tn}, TC=0.1%)",
         d_ret, colors["Daily"]),
    ]:
        w = (1+s).cumprod(); w = w/w.iloc[0]
        ax1.plot(w.index, w.values, label=label, color=col, linewidth=2)

    ax1.set_title("Good Retail Execution: Hourly vs Daily Momentum (TC=0.1%)", fontsize=13)
...
xl = "results/momentum_retail_comparison.xlsx"
try:
...
_header("FINAL SUMMARY (Good Retail: TC=0.1% one-way)")
