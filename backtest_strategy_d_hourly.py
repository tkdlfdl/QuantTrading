...
print("=" * 120)
print("STRATEGY D: CONTRARIAN BUBBLE SCORE - HOURLY BACKTEST (2019-2026)")
print("=" * 120)
print("\nParameters (EXACT from CONTRARIAN_BUBBLE_STRATEGY.md):")
print("  Bubble MA Window: 104 hours")
print("  Threshold: -0.8")
print("  Hold Period: 13 hours")
...
try:
    ...
    print(f"ERROR: Could not load hourly data: {e}")
    print("Checking available files...")
    import os
    files = os.listdir('data/cache/')
    print("Available files:", [f for f in files if 'hourly' in f.lower()])
    exit(1)

# Filter to 2019-2026
hourly_close = hourly_close.loc['2019-01-01':'2026-06-12'].copy()
hourly_open = hourly_open.loc['2019-01-01':'2026-06-12'].copy()

# Drop columns with too much missing data
hourly_close = hourly_close.dropna(axis=1, thresh=len(hourly_close) * 0.70)
hourly_open = hourly_open.dropna(axis=1, thresh=len(hourly_open) * 0.70)
...
print(f"\n[Backtesting with Hourly Data]")
print(f"  MA Window: {MA_WINDOW} hours")
print(f"  Hold Period: {HOLD_PERIOD} hours")
print(f"  Z-Score Window: {Z_SCORE_WINDOW} hours")
...
for i in range(MA_WINDOW + Z_SCORE_WINDOW, len(hourly_close)):
    ...
    for col in hourly_close.columns:
        try:
            ...
            log_p = np.log(recent_prices.replace(0, np.nan).fillna(method='ffill'))
    ...
    # Select top-20 with lowest bubble scores
    active_positions = list(position_tracker.keys())
    active_returns = []

    # Calculate returns from active positions
    ...
    # Calculate daily return from all active positions
    try:
...
# Hourly returns CSV
hourly_returns.to_csv('results/Strategy_D_Hourly_Returns_2019_2026.csv', header=['Hourly_Return'])
print(f"  [OK] Strategy_D_Hourly_Returns_2019_2026.csv ({len(hourly_returns):,} bars)")

# Daily returns CSV
daily_returns_calc.to_csv('results/Strategy_D_Daily_Returns_Hourly_2019_2026.csv', header=['Daily_Return'])
print(f"  [OK] Strategy_D_Daily_Returns_Hourly_2019_2026.csv ({len(daily_returns_calc):,} days)")

# Yearly metrics CSV
yearly_df = pd.DataFrame(yearly_results)
yearly_df.to_csv('results/Strategy_D_Yearly_Metrics_Hourly_2019_2026.csv', index=False)
print(f"  [OK] Strategy_D_Yearly_Metrics_Hourly_2019_2026.csv")
...
summary.to_csv('results/Strategy_D_Summary_Hourly_2019_2026.csv', index=False)
print(f"  [OK] Strategy_D_Summary_Hourly_2019_2026.csv")

print(f"\n{'='*120}")
print(f"STRATEGY D HOURLY BACKTEST COMPLETE")
print(f"{'='*120}")
