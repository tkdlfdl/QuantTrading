"""
Book C with PER-TICKER OVERLAP CAP — Improvement Cycle 1 (queue item #1).

Identical to retest_book_c_mtm.py except: a new trade SKIPS any ticker still
held by an open trade (no same-ticker exposure stacking). Motivated by the
verifier finding: consecutive 4-sigma signals on HOOD (2021-08-02/03) stacked
~2x single-stock exposure -> +93%/-54% days -> the -53.7% MaxDD.
...
n_trades = 0
busy = {}                          # ticker -> day-index until which it is held
n_skipped = 0
...
for sig_date, exec_date in zip(days[:-1], days[1:]):
    ...
    if not positions:
        continue
    exec_i_pre = day_to_i[exec_date]
    held = [t for t in positions if busy.get(t, -1) >= exec_i_pre]
    if held:
        n_skipped += len(held)
        for t in held:
            positions.pop(t)
...
record_performance(
    name="book_c_overlap_cap", dates=ser.index, returns=ser.values,
    ...
    periods_per_year=TD,
    extra={"convention": "PROPER daily MTM + per-ticker overlap cap", "cycle": "Improvement Cycle 1",
           "n_trades": n_trades})
print("recorded -> strategies/performance/book_c_overlap_cap_*")
